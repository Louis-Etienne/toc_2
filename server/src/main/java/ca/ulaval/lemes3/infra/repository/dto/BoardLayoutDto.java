...
public class BoardLayoutDto {
    ...
    public BoardLayout toDomain(){
        MarbleId[] trackDomain = Arrays.stream(track)
                .map(id -> id == null ? null : new MarbleId(id))
                .toArray(MarbleId[]::new);
        Map<PlayerId, List<MarbleId>> homesDomain = homes.entrySet().stream()
                .collect(Collectors.toMap(
                        entry -> new PlayerId(entry.getKey()),
    ...
    public BoardLayoutDto(BoardLayout boardLayout){
        track = Arrays.stream(boardLayout.getTrack())
                .map(marbleId -> marbleId == null ? null : marbleId.id())
                .toArray(UUID[]::new);

        homes = boardLayout.getHomes().entrySet().stream()
                .collect(Collectors.toMap(
        ...
        heavens = boardLayout.getHeavens().entrySet().stream()
                .collect(Collectors.toMap(
                        entry -> entry.getKey().id(),
                        entry -> entry.getValue().stream()
                                .map(marbleId -> marbleId == null ? null : marbleId.id())
